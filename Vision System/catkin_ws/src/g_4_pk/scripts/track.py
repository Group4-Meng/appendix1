#!/usr/bin/env python3
#python track_webcam.py --weights best.pt
...
def detect(m):
	...
	sort_tracker = Sort(max_age=sort_max_age,min_hits=sort_min_hits,iou_threshold=sort_iou_thresh) # {plug into parser}
		...
		for i, det in enumerate(pred):  # detections per image
			...
			if len(det):
				...
				if len(tracked_dets)>0:
					...
					draw_boxes(frame, bbox_xyxy, identities, categories, names)
					c1, c2 = (int(bbox_xyxy[0][0]), int(bbox_xyxy[0][1])), (int(bbox_xyxy[0][2]), int(bbox_xyxy[0][3]))
					centre=(int((c1[0]+c2[0])/2),int((c1[1]+c2[1])/2))
					print(centre)
					centre_list.append(centre)
					class_list.append(categories)
